...
function stopScroll() {
    ...
    // Função para criar e adicionar assertividade
    function addAssertividade(contextOptions) {
        ...
        // Gera assertividade entre 1% e 100%
        const assertividadeValue = Math.random() * 99 + 1;
        ...
        if (assertividadeValue < 90) {
            assertividadeElement.style.color = 'red'; // Menos que 90% em vermelho
        } else {
            assertividadeElement.style.color = 'green'; // 90% ou mais em verde
        }

        contextOptions.appendChild(assertividadeElement);
    }

    // Função para criar e adicionar imagem aleatória
    function addRandomImage(contextOptions) {
    ...
    // Função para remover assertividade e imagem
    function removeAssertividadeAndImage(contextOptions) {
    ...
    // Mostrar animação de carregamento e aguardar 5 segundos
    showLoadingAnimation();
    setTimeout(() => {
        hideLoadingAnimation();

        // Exibir assertividade e imagem aleatória
        const contextOptions = document.getElementById('contextOptions');
        if (contextOptions) {
            addAssertividade(contextOptions);
            addRandomImage(contextOptions);

            // Reverter para o estado inicial após mais 5 segundos
            setTimeout(() => {
                if (contextOptions) {
                    removeAssertividadeAndImage(contextOptions);
                }
